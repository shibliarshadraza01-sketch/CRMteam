"""CP18: lightweight regression checks that CP1-CP17 remain intact. Not a
re-run of their own suites — this only guards against the specific way a
new app (apps.integrations) could accidentally break something already
built, especially given this checkpoint reuses CP4's password-hashing
infrastructure directly. All DB-free.
"""
...
def test_integrations_app_is_registered_in_the_app_registry():
    from django.apps import apps

    assert apps.is_installed("apps.integrations")
    config = apps.get_app_config("integrations")
    assert config.name == "apps.integrations"
...
def test_cp4_password_hashing_helpers_still_importable():
    """This checkpoint's API key hashing reuses Django's own
    make_password()/check_password() — the same functions CP4's
    User.set_access_code()/check_access_code() already use. Confirms
    that reuse path still resolves.
    """
    from django.contrib.auth.hashers import check_password, make_password

    hashed = make_password("test-value")
    assert check_password("test-value", hashed) is True
    assert check_password("wrong-value", hashed) is False
...
def test_openapi_schema_still_documents_every_pre_cp18_endpoint():
    ...
    for path in (
        ...
        assert path in paths, f"{path} missing from schema after CP18"
